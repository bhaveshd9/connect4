...
unsigned int COMPUTER = 2; // computer number
...
void playGame() {
	...
	while (!gameOver) { // while no game over state
		if (currentPlayer == COMPUTER) { // computer move
			makeMove(board, computerMove(), COMPUTER);
		}
	...
	if (turns == NUM_ROW * NUM_COL) { // if draw condition
		...
		cout << ((currentPlayer == PLAYER) ? "Computer Wins!" : "Player Wins!") << endl;
	}
}
...
int userMove() {
    ...
    while (true) { // repeat until proper input given
        ...
        if (!((unsigned int)move < NUM_COL && move >= 0)) { // if out of bounds
 ...
 */
int computerMove() {
	cout << "Computer is thinking about a move." << endl;
...
array<int, 2> miniMax(vector<vector<int> > &b, unsigned int d, int alf, int bet, unsigned int p) {
	...
	if (p == COMPUTER) { // if computer player
...
int tabScore(vector<vector<int> > b, unsigned int p) {
	 ...
	 * containing any combination of computer, player, and empty pieces
	 */
	for (unsigned int r = 0; r < NUM_ROW; r++) {
		for (unsigned int c = 0; c < NUM_COL; c++) {
...
void printBoard(vector<vector<int> > &b) {
	...
	for (unsigned int r = 0; r < NUM_ROW; r++) {
		for (unsigned int c = 0; c < NUM_COL; c++) {
			...
			case 2: cout << "X"; break; // computer's piece
			}
			if (c + 1 == NUM_COL) { cout << "|"; }
		}
		cout << endl;
...
void errorMessage(int t) {
	if (t == 1) { // non-int input
		cout << "Use a valid value 0-" << NUM_COL - 1 << endl;
	}
